<?php   										// Opening PHP tag
	 ...
	 */
	 ...
	 function get_all_toy_info(PDO $pdo, string $toyId) {
    $sql = "SELECT *
            FROM toy
    ...
    // Execute the SQL query using the pdo function and fetch the result
	$toy = pdo($pdo, $sql, ['id' => $toyId])->fetch();		// Associative array where 'id' is the key and $id is the value. Used to bind the value of $id to the placeholder :id in  SQL query.

	// Return the toy information (associative array)
	return $toy;
}
$toy_info = get_all_toy_info($pdo, $toy_id);
// Closing PHP tag  ?> 
		...
		<main>
			...
			<div class="toy-details-container">
				...
				<div class="toy-details">
			        ...
			        <p><strong>Address:</strong> <?= $toy_info['Street'] . ', ' . $toy_info['City'] 
					. ', ' . $toy_info['State'] . ',' . $toy_info['ZipCode'] ?></p>
