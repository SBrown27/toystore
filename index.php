<?php   										// Opening PHP tag
	 ...
	 */
	/*
	* Retrieve information for ALL toys from the database.
	* 
	* @param PDO $pdo 	An instance of the PDO class.
	* @return array|Null 	An array of associative arrays, each containing the toy information, 
	* or null if no toy is found.
	*/
	function get_remaining_toys(PDO $pdo) {
		// Making an array to hold the toys
		$toys = [];
    	for ($i = 2; $i <= 10; $i++) 
		{
			//	// Need to get all the id's in the proper formatting
			$formattedId = sprintf('%04d', $i);
			//	Getting each toy. Using get_toy for this.
			$toys["toy$i"] = get_toy($pdo, $formattedId);
    	}
    	return $toys;
	}

// Retrieve information for all toys from the database using provided PDO connection
$remaining_toys = get_remaining_toys($pdo);

// Iterating through the $remaining_toys array and setting each variable dynamically
for ($i = 2; $i <= 10; $i++) {
    ${"toy$i"} = $remaining_toys["toy$i"]; // Getting each toy
}
