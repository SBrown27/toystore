<?php   										// Opening PHP tag
	 ...
	 */
	function get_order_and_info(PDO $pdo, $email, $orderNum) {
		$sql = "SELECT *
				FROM orders
		...
		// Execute the SQL query using the pdo function and fetch the result
		$info = pdo($pdo, $sql, ['email' => $email,'orderNum'=>$orderNum])->fetch();		// Associative array where 'id' is the key and $id is the value. Used to bind the value of $id to the placeholder :id in  SQL query.
		return $info;
	}
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		$orderInfo = get_order_and_info($pdo, $email, $orderNum);
